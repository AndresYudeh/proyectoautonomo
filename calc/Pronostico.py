...
class Pronostico:
    ...
    def promedioMovil(self, x:str, y:str, xlbl:str, ylbl:str):
        ...
        plt.grid()
        plt.plot(a[ylbl],label=ylbl,marker='o', color='black')
        plt.plot(a['MMO_3'],label=xlbl, marker='*', color='purple')
    ...
    def suavizacionExponencial(self, x:str, y:str, xlbl:str, ylbl:str):
        ...
        for i in range(0,movil.shape[0]-1):
            movil.loc[movil.index[i+1],'S_N'] = np.round(movil.iloc[i,1],1)
        for i in range(2,movil.shape[0]):
            movil.loc[movil.index[i],'S_N'] = np.round(movil.iloc[i-1,1],1)*alfa + np.round(movil.iloc[i-1,2],1)*unoalfa
        i=i+1
        intervalo = datosX[1]-datosX[0]
        p1=0
        p2=np.round(movil.iloc[i-1,1],1)*alfa + np.round(movil.iloc[i-1,2],1)*unoalfa
        a = movil.append({xlbl:datosX[-1]+intervalo,ylbl:p1, 'S_N':p2},ignore_index=True)
    ...
    def regresionLineal(self, x:str, y:str, xlbl:str, ylbl:str):
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.legend(('Datos','Recta',), loc="upper left")
        plt.savefig(img, format='png')
        plt.clf()
        img.seek(0)
    ...
    def regresionLinealCuadratica(self, x:str, y:str, xlbl:str, ylbl:str):
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.legend(['Dato n','Función'], loc="upper left")
        plt.savefig(img, format='png')
        img.seek(0)
        img_url = base64.b64encode(img.getvalue()).decode()
        return img_url
